...
namespace Senparc.Toutiao.Cache
{
    ...
    public interface IContainerCacheStrategy : IDomainExtensionCacheStrategy /*: IBaseCacheStrategy<string, IBaseContainerBag>*/
    {
        ...
        /// <typeparam name="TBag"></typeparam>
        /// <returns></returns>
        Task<IDictionary<string, TBag>> GetAllAsync<TBag>() where TBag : IBaseContainerBag;
        ...
        /// <param name="isFullKey">是否已经是完整的Key，如果不是，则会调用一次GetFinalKey()方法</param>
        /// <returns></returns>
        Task<TBag> GetContainerBagAsync<TBag>(string key, bool isFullKey = false) where TBag : IBaseContainerBag;
