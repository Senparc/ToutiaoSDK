...
using Senparc.Toutiao.Exceptions;
...
{
    //TODO：将ToutiaoTrace和SenparcTrace通过某种标记明显区分开来

    /// <summary>
    /// 微信日志跟踪
    /// </summary>
    public class ToutiaoTrace : SenparcTrace
    {
        /// <summary>
        /// 记录WeixinException日志时需要执行的任务
        /// </summary>
        public static Action<ToutiaoException> OnWeixinExceptionFunc;
        ...
        /// </summary>
        /// <param name="ex"></param>
        public static void ToutiaoExceptionLog(ToutiaoException ex)
        {
            if (!Config.IsDebug)
            {
                return;
            }
            using (var traceItem = new SenparcTraceItem(SenparcTrace._logEndActon, "ToutiaoException"))
            {
